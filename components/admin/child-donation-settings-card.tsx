...
export function ChildDonationSettingsCard({ settings }: { settings: Settings | null }) {
  ...
  return <>
    <article className="flex min-h-[25rem] min-w-0 flex-col rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface-raised)] p-5">
      <div className="mb-4 flex min-h-8 justify-end">
        <span className={`inline-flex min-h-8 items-center gap-2 rounded-full border px-3 text-xs font-bold uppercase tracking-wide ${settings ? "border-emerald-300 bg-emerald-100 text-emerald-800" : "border-[var(--admin-border)] bg-[var(--admin-surface)] text-[var(--admin-muted)]"}`}>
          <span aria-hidden="true" className="size-2 rounded-full bg-current" />
          {settings ? "Aktif" : "Ayar bekliyor"}
        </span>
      </div>
      <div className="flex flex-col items-start gap-3">
        <div className="flex min-w-0 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-lg bg-[#e8f2ea]"><Image alt="Ahmet" height={38} src="/images/orphan/kid/head_happy.svg" width={38} /></span><div><p className="break-words text-base font-semibold text-[var(--admin-text)]">Ahmet&apos;e Destek</p><p className="mt-1 text-xs text-[var(--admin-muted)]">Sabit paket bağışı · Çoklu para birimi</p></div></div>

      </div>
      <dl className="mt-5 mb-5 grid grid-cols-1 gap-3"><div className="rounded-lg bg-[var(--admin-surface)] p-3"><dt className="text-[11px] text-[var(--admin-muted)]">Paket türleri</dt><dd className="mt-1 text-sm font-semibold">4 paket</dd></div><div className="rounded-lg bg-[var(--admin-surface)] p-3"><dt className="text-[11px] text-[var(--admin-muted)]">Para birimleri</dt><dd className="mt-1 font-mono text-sm font-semibold">TRY · USD · EUR</dd></div></dl>
      <button className="admin-action-button mt-auto w-full justify-center" onClick={() => setOpen(true)} type="button"><Pencil className="size-4" />Kampanyayı düzenle</button>
    </article>
