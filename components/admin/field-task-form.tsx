...
function SubmitButton({ isEditing }: { isEditing: boolean }) {
  ...
  return (
    <button className="admin-action-button sm:w-auto" disabled={pending} type="submit">
      {pending ? <LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> : isEditing ? <Pencil aria-hidden="true" className="size-4" /> : <ClipboardPlus aria-hidden="true" className="size-4" />}
      {pending ? "Kaydediliyor" : isEditing ? "Görevi güncelle" : "Görev oluştur"}
    </button>
  );
}
    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-semibold text-[var(--admin-text)]">
        <span className="flex items-center gap-2">
          ...
          {isEditing ? "Görevi düzenle" : "Yeni saha görevi"}
        </span>
      ...
      <form action={formAction} className="border-t border-[var(--admin-border)] p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label>
            <span className="mb-2 block text-xs font-semibold text-[var(--admin-muted)]">Görev başlığı *</span>
            <input className="admin-input" defaultValue={record?.title || ""} name="title" required type="text" />
          </label>
          <label>
            <span className="mb-2 block text-xs font-semibold text-[var(--admin-muted)]">Konum *</span>
            <input className="admin-input" defaultValue={record?.location || ""} name="location" required type="text" />
          </label>
          <label>
            <span className="mb-2 block text-xs font-semibold text-[var(--admin-muted)]">Bağış alanı *</span>
            <select className="admin-input" defaultValue={record?.campaign || ""} name="campaign" required>
          ...
          <label>
            <span className="mb-2 block text-xs font-semibold text-[var(--admin-muted)]">Saha görevlisi *</span>
            <select className="admin-input" defaultValue={record?.assignedTo || ""} name="assignedTo" required>
              <option disabled value="">
                Seçin
              </option>
              {operators.map((item) => (
          ...
          <label className="sm:col-span-2">
            ...
            <textarea className="admin-input min-h-24 resize-y" defaultValue={record?.notes || ""} name="notes" />
          </label>
        </div>
        {state.message ? (
