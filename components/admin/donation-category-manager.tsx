    ...
    <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-3xl bg-[var(--admin-surface-raised)] p-5 shadow-2xl sm:max-w-3xl sm:rounded-3xl sm:p-7">
      ...
      <form action={action} className="mt-5 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div><label><span className="admin-label">Kategori simgesi</span><input className="admin-input" onChange={(event) => setForm({ ...form, icon: event.target.value })} value={form.icon} /></label><div className="mt-2 flex flex-wrap gap-2">{[["qurbani", "Kurban"], ["water_drop", "Su"], ["emergency", "Acil yardım"], ["child_care", "Yetim"], ["school", "Eğitim"], ["favorite", "Genel"]].map(([icon, label]) => <button aria-label={label} title={label} aria-pressed={form.icon === icon} className={`grid size-10 place-items-center rounded-lg border text-xl ${form.icon === icon ? "border-orange-400 bg-orange-50 text-orange-600" : "border-[var(--admin-border)]"}`} key={icon} type="button" onClick={() => setForm({ ...form, icon })}><DonationCategoryIcon name={icon} /></button>)}</div></div>
...
export function DonationCategoryManager({ categories }: { categories: DonationCategoryAdminRecord[] }) {
  ...
  return <>
    ...
    {ordered.length ? <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{ordered.map((category) => <article className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface-raised)] p-5 shadow-sm" key={category.id}>
      <div className="flex items-start justify-between gap-3"><span className="grid size-12 place-items-center rounded-xl text-white" style={{ backgroundColor: category.color }}><DonationCategoryIcon name={category.icon || "category"} className="text-2xl" /></span><span className={category.isActive ? "admin-status admin-status-success" : "admin-status admin-status-neutral"}>{category.isActive ? "Aktif" : "Pasif"}</span></div>
