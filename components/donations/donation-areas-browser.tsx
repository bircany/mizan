...
// Finansal ilerleme sadece yönetim panelinde gösterilir; ziyaretçi kartlarında yer almaz.
function Stat(_props: { label: string; value: string }) {
  return null;
}

function campaignDefaultAmount(area: DonationArea) {
  return area.pricingModel === "fixed"
    ? Number(area.unitPrice || 0)
    : area.suggestedAmounts[1] || area.suggestedAmounts[0] || 100;
}

function AreaCard({ area, locale, onDonate }: { area: DonationArea; locale: AppLocale; onDonate: (amount: number) => void }) {
  const text = labels[locale];
  const defaultAmount = campaignDefaultAmount(area);
    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[24px] border border-outline-variant/20 bg-white shadow-[0_8px_24px_rgba(28,42,30,.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_34px_rgba(28,42,30,.13)]">
      <div className="relative aspect-[4/3] overflow-hidden"><CoverImage area={area} />
        <div className="absolute left-3 top-3 flex max-w-[calc(100%-1.5rem)] gap-2">
          {area.category ? <span className="inline-flex min-w-0 items-center gap-1.5 rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-primary shadow-sm backdrop-blur"><DonationCategoryIcon name={area.category.icon || "volunteer_activism"} className="text-[15px]" /><span className="truncate">{area.category.name}</span></span> : null}
        </div>
      </div>
      <div className="flex flex-1 flex-col p-5">
        <div className="min-w-0"><h2 className="line-clamp-2 text-xl font-semibold leading-snug text-on-surface">{area.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-6 text-on-surface-variant/70">{area.excerpt}</p></div>
        {area.pricingModel === "fixed" ? <p className="mt-5 rounded-xl bg-surface-container-low px-3 py-2 text-sm font-semibold text-on-surface">{formatCurrency(defaultAmount, area.currency)} / {area.unitLabel || "adet"}</p> : <div className="mt-5 grid grid-cols-3 gap-2">{area.suggestedAmounts.slice(0, 3).map((amount) => <button className="rounded-xl border border-outline-variant/30 bg-surface-container-low px-2 py-2 text-xs font-semibold text-on-surface transition hover:border-secondary hover:bg-secondary/10 hover:text-secondary" key={amount} onClick={() => onDonate(amount)} type="button">{formatCurrency(amount, area.currency)}</button>)}</div>}
...
export function DonationAreasBrowser({ areas, categories, locale }: Props) {
  ...
  const formatPrice = (amount: number) => formatCurrency(amount, items[0]?.currency || "TRY");
