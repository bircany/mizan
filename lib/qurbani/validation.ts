...
export function parseCreateQurbaniOrder(
  value: unknown,
): CreateQurbaniOrderInput {
  if (!value || typeof value !== "object")
    throw new Error("Siparis bilgileri gecersiz.");
  const body = value as Record<string, any>;
  const shareCount = Number(body.shareCount);
  const productId = text(body.productId, 40);
  const rawBuyer =
    body.buyer && typeof body.buyer === "object" ? body.buyer : {};
  const buyerCountryCode = text(rawBuyer.countryCode, 2).toUpperCase();
  const buyer = {
    firstName: text(rawBuyer.firstName, 80),
    lastName: text(rawBuyer.lastName, 80),
    email: text(rawBuyer.email, 180).toLowerCase(),
    phone: normalizeInternationalPhone(rawBuyer.phone, buyerCountryCode),
  ...
  const shareholders = Array.isArray(body.shareholders)
    ? body.shareholders.slice(0, 7).map((item: any) => {
        const phoneCountryCode =
          text(item?.phoneCountryCode, 2).toUpperCase() || buyerCountryCode;
        return {
          name: text(item?.name, 120),
          phone: optionalInternationalPhone(item?.phone, phoneCountryCode),
          phoneCountryCode,
        };
      })
    : [];
  const consents =
    body.consents && typeof body.consents === "object" ? body.consents : {};
  ...
  if (
    ...
    shareCount < 1 ||
    shareCount > 7 ||
    shareholders.length !== shareCount
  ) {
    throw new Error("Urun ve hisse sayisi gecersiz.");
  }
  if (shareholders.some((item) => item.name.length < 2))
    throw new Error("Her hisse sahibi icin ad soyad zorunludur.");
  if (
    !buyer.firstName ||
    !buyer.lastName ||
  ...
  if (!buyer.identityNumber)
    throw new Error("Kimlik veya pasaport numarasi zorunludur.");
  if (
  ...
  if (
    buyer.countryCode !== "TR" &&
    !/^[A-Za-z0-9][A-Za-z0-9 -]{4,31}$/.test(buyer.identityNumber)
  ) {
    throw new Error("Pasaport veya ulusal kimlik numarasi gecersiz.");
  }
  if (body.paymentMethod !== "iyzico" && body.paymentMethod !== "eft")
    throw new Error("Odeme yontemi gecersiz.");
  if (!consents.digitalPowerOfAttorney || !consents.terms || !consents.kvkk)
    throw new Error("Vekalet, KVKK ve bagis sartlari onaylanmalidir.");
  if (shareholders.some((item) => item.phone) && !consents.thirdPartyContact) {
    throw new Error(
      "Ucuncu kisi iletisim bilgileri icin bilgilendirme onayi zorunludur.",
    );
  }
